...
int found=0;
    private NewInvoice1 parentInvoice; // Reference to the NewInvoice instance
    private UpdateInvoice1 updateInvoiceParent; // Reference to the NewInvoice instance

    private String searchQuery;

    public itemSearch(NewInvoice1 parent, String searchQuery) {
        initComponents();
        centerFrame();
    ...
    public itemSearch(UpdateInvoice1 parent, String searchQuery, int x) {
        found=1;
        initComponents();
        centerFrame();
    ...
    private void populateTable() {
        ...
        try {
            ...
            ResultSet rs = st.executeQuery("SELECT `ID`, `Description`, `Quantity`, `SellingPrice` FROM item WHERE ID LIKE '%" + searchQuery + "%' OR Description LIKE '%" + searchQuery + "%'");
            while (rs.next()) {
                int itemID = rs.getInt("ID");
                String description = rs.getString("Description");
                double quantity = rs.getDouble("Quantity");
                double unitPrice = rs.getDouble("SellingPrice");
                model.addRow(new Object[]{itemID, description, quantity, unitPrice});
            }
            st.close();
            con.close();
        } catch (Exception e) {
        ...
        if (selectedRow >= 0) {
            ...
            if (parentInvoice != null) {
                parentInvoice.addDataToTable(itemId, itemName,"0" ,unitPrice,"0");
                 parentInvoice.jTextField5.setText(availQty);
            }
            parentInvoice.jTextField4.setText("");
